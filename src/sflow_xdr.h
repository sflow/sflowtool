...
#define SFD_ASSERT(x)
  ...
  static SFDBuf *SFDSampleNew(SFDDgram *sfdg) {
    ...
    if(buf)
     ...
     buf = (SFDBuf *)sfdg->f_alloc(sfdg->cb_magic, sizeof(SFDBuf));
    sfd_xdr_init(buf);
    return buf;
  }

  static void SFDSampleFree(SFDDgram *sfdg, SFDBuf *buf) {
    buf->nxt = sfdg->bufs;
    sfdg->bufs = buf;
  }
  ...
  static void SFDInit(SFDDgram *sfdg,
		      uint32_t maxDgramLen,
		      SFLAddress *agentAddress,
		      uint32_t agentSubId,
		      void *cb_magic,
    ...
    sfdg->cb_magic = cb_magic;
    sfdg->f_alloc = allocFn;
    sfdg->f_free = freeFn;
    sfdg->f_now_mS = nowFn;
    sfdg->f_send = sendFn;
    sfdg->bootTime_mS = sfdg->f_now_mS(sfdg->cb_magic);
  ...
  static void SFDReset(SFDDgram *sfdg) {
    // Recycle bufs.
    for(uint32_t ii=0; ii<sfdg->nsamples; ii++)
      SFDSampleFree(sfdg, sfdg->samples[ii]);
  ...
  static void SFDSend(SFDDgram *sfdg) {
    ...
    if(sfdg->nsamples == 0)
      return;
    // Get timestamp.
    sfdg->lastSend_mS = sfdg->f_now_mS(sfdg->cb_magic);
    // Complete the header.
    SFDBuf *hdr = &(sfdg->hdr);
    ...
    // Send out datagram.
    sfdg->f_send(sfdg->cb_magic, sfdg->iov, sfdg->nsamples + 1);
    // Reset.
    SFDReset(sfdg);
  }
